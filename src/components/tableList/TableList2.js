...
import CountryCreateModal from '../modal/CountryCreateModal'
import './style.css';
...
export default function TableList2(props) {
    ...
    async function swalFire(data) {
        ...
        try {
            ...
            const request = await PostApi('/Country/Delete',data);         console.log(request
    )   
            if (request.status === 1) {
    ...
    const checkEventsAvailable = (cIndex) => {
        if (isEventsAvailable()) {
            return(<th key={cIndex} className="min-w-50px sorting sorting_asc">عملیات</th>)
        }
    };
                    ...
                    <tr className="fw-bolder text-muted bg-light">
                        {
                            config.columns.map((column, cIndex) => {
                                return(
                                    <th key={cIndex} onClick={() => requestSort(column['dataField'])} className={getHeaderClassName(column, cIndex)}>{column.caption}</th>
                                )
                            })
                        }
